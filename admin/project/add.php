...
<div class="container-fluid">
    ...
    <form method="post" enctype="multipart/form-data">
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="usr" name="txtname" value="<?php echo $name ?>" placeholder="Ví dụ: Mái ấm tình thương...">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="usr" name="location" value="<?php echo $location ?>" placeholder="Ví dụ: TP.Hồ Chí Minh...">
        </div>
        <div class="form-group">
            <label for="usr">Ngày bắt đầu kêu gọi</label>
            <input type="date" class="form-control" id="usr" name="startdate" value="<?php echo $startdate == "" ? "" : date("Y-m-d", strtotime($startdate)) ?>">
        </div>
        <div class="form-group">
            <label for="usr">Ngày kết thúc kêu gọi</label>
            <input type="date" class="form-control" id="usr" name="enddate" value="<?php echo $enddate == "" ? "" : date("Y-m-d", strtotime($enddate)) ?>">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" rows="5" id="comment" name="description"  placeholder="Ví dụ: Giúp đỡ hoàn cảnh khó khăn, vô gia cư..."><?php echo $description ?></textarea>
        </div>
        <div class="form-group">
            <label for="pwd">Mô tả chi tiết</label>
            <textarea class="form-control" id="inp_editor1" rows="5" id="comment" name="detailed_description"><?php echo $detailed_description ?></textarea>
        </div>
        <div class="form-check">
            <label class="form-check-label" for="check1">
                <input type="checkbox" class="form-check-input" id="check1" name="checktotal" value="1" checked>Tự động đóng dự án khi đủ số tiền
            </label>
        </div>
        <div class="form-check">
            <label class="form-check-label" for="check2">
                <input type="checkbox" class="form-check-input" id="check2" name="checktime" value="1" checked>Tự động đóng dự án khi hết thời hạn quyên góp
            </label>
        </div>
        <?php if (count($err) > 0) { ?>
